...
# -----------------------where-------------
medb_fin_opp_product_rh_merge = medb_fin_opp_product_rh_merge[
    (medb_fin_opp_product_rh_merge['SPECIALITY_PHCY_IND'] == 1) &
...
MEDB_CLAIMPULL_v2 = MEDB_CLAIMPULL_v1[["PATIENT_ID", "PHCY_PRODUCT_SERVI CE_ID", "GCN_NBR", "SERVICED_DTE"]]
...
MEDB_WIDGET_WORK["STATUS_CDE"] = np.where(MEDB_WIDGET_WORK['PHCY_CLAIM_ID'].isin(OTH_FIN_WIDGET_MERGE_lst),
                                          STATUS_CDE='D',
                                          MEDB_WIDGET_WORK["STATUS_CDE"])
...
MEDB_WIDGET_WORK["STATUS_CDE"] = np.where(MEDB_WIDGET_WORK['PHCY_CLAIM_ID'].isin(BWP_FIN_WIDGET_MERGE_lst),
                                          STATUS_CDE='D',
                                          MEDB_WIDGET_WORK["STATUS_CDE"])
...
MEDB_WIDGET_WORK["STATUS_CDE"] = np.where(
    MEDB_WIDGET_WORK['PHCY_CLAIM_ID'].isin(CURR_PHARMACY_SUBMIT_TYPE_FIN_WIDGETV1),
    STATUS_CDE='D', MEDB_WIDGET_WORK["STATUS_CDE"])
...
# ---------------------7a second part---------incomplete-------
#-----------------updating-----------------
# ----------------merge2---
clm_grp_dod_member_current_merge = MEDB_CLM_GRP.merge(DOD_MEMBER_CURRENT, how="inner",
                                                      on=["PATIENT_ID", "GROUP_OPERATIONAL_ID"]
print("----------clm_grp_dod_member_current_merge---\n", clm_grp_dod_member_current_merge.shape)
print("----------clm_grp_dod_member_current_merge---\n", clm_grp_dod_member_current_merge)

# -----where condition----
clm_grp_dod_member_current_merge_v1 = clm_grp_dod_member_current_merge[
    (clm_grp_dod_member_current_merge["EFF_DTE"] < DATE + 1) &
...
# ---------------------------PAT_GRP_NEWELIG-------------incomplete------------7 b started------
# --------------replace----------
...
# --------------rename a column name--
MEDB_PATGRP_UNQMAX_CLM.rename(columns={"GROUP_OPERATIONAL_ID": "GROUP_OPERATIONAL_ID_v1"}, inplace=True)
...
hiererchy.rename(columns={"CARRIER_OPEARYIONAL_ID": "CARRIER_OPEARYIONAL_ID_v1"}, inplace=True)
hiererchy.rename(columns={"CONTRACT_OPERATIONAL_ID": "CONTRACT_OPERATIONAL_ID_v1"}, inplace=True)
...
#-------------------------merge 2--------
claimpull_unqmax_clm_hierar_merge=claimpull_unqmax_clm_merge.merge(hiererchy,how="inner",on="GROUP_OPERATIONAL_ID")
...
# # --------------rename a column name--
claimpull_unqmax_clm_hierar_merge_final_v1.rename(columns={"GROUP_OPERATIONAL_ID_v1": "GROUP_OPERATIONAL_ID"}, inplace=True)
...
#------------------------------------------------logic 8  started------------
#---------------MEDB_FORM_TMP--------------
MEDB_FORM_TMP1=MEDB_CLAIMPULL[["ELIG_GROUP_OPERATIONAL_ID","ADJUD_GROUP_OPERATIONAL_ID","PRODUCT_ID","MAIL_RETAIL_CDE","PRF_NDC"]]
print("-------MEDB_FORM_TMP1--\n", MEDB_FORM_TMP1.shape)
print("-------MEDB_FORM_TMP1--\n", M   EDB_FORM_TMP1)
print("-------MEDB_FORM_TMP1--\n", MEDB_FORM_TMP1.columns)

#-----------rename a column names------
MEDB_FORM_TMP1.rename(columns={"ELIG_GROUP_OPERATIONAL_ID": "GROUP_OPERATIONAL_ID"}, inplace=True)
MEDB_FORM_TMP1.rename(columns={"PRODUCT_ID": "MAIL_RETAIL_CDE"}, inplace=True)
MEDB_FORM_TMP1.rename(columns={"MAIL_RETAIL_CDE": "MAIL_RETAIL_CDE2"}, inplace=True)
MEDB_FORM_TMP1.rename(columns={"PRF_NDC": "ALT_PRODUCT_SERVICE_ID"}, inplace=True)
...
MEDB_GRP_CHNL_V1=MEDB_GRP_CHNL[["GROUP_OPERATIONAL_ID","PRODUCT_ID"]]
...
#------------MERGE 1------
medb_pro_itemrh_merge=MEDB_GRP_CHNL_V1.merge(PRODUCT_ITEM_RH,how="inner",on="PRODUCT_ID")
print("-------medb_pro_itemrh_merge---\n",medb_pro_itemrh_merge.shape)
print("-------medb_pro_itemrh_merge---\n",medb_pro_itemrh_merge)
print("-------medb_pro_itemrh_merge---\n",medb_pro_itemrh_merge.columns)
#---------------MERGE2-----------------------
medb_pro_itemrh_clientrh_merge=medb_pro_itemrh_merge.merge(CLIENT_RH,how="inner",on="GROUP_OPERATIONAL_ID","CLIENT_ID")
print("-------medb_pro_itemrh_clientrh_merge---\n",medb_pro_itemrh_clientrh_merge.shape)
print("-------medb_pro_itemrh_clientrh_merge---\n",medb_pro_itemrh_clientrh_merge)
print("-------medb_pro_itemrh_clientrh_merge---\n",medb_pro_itemrh_clientrh_merge.columns)
...
#---------------------------------logic 9 starts 9:58--------vedio ends in 10------
#-----------------logic 9 started----------------
